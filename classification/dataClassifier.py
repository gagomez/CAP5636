            ...
            if datum.getPixel(x, y) == 0 and (x, y) not in visited:
                ...
                while not Q.isEmpty():
                    ...
                    for dx in [-1, 1]:
                        npx = px + dx
                        npy = py
                        if npx >= 0 and npx < DIGIT_DATUM_WIDTH:
                            np = (npx, npy)
                            if datum.getPixel(npx, npy) == 0 and np not in visited:
                                visited.add(np)
                                Q.push(np)
                    for dy in [-1, 1]:
                        npx = px
                        npy = py + dy
                        if npy >= 0 and npy < DIGIT_DATUM_HEIGHT:
                            np = (npx, npy)
                            if datum.getPixel(npx, npy) == 0 and np not in visited:
                                visited.add(np)
                                Q.push(np)
...
def enhancedPacmanFeatures(state, action):
    ...
    ghostDistances = [util.manhattanDistance(gp, successor.getPacmanPosition()) for gp in ghostPositions]
    ghostDistances.append(2000)

    capsulePositions = successor.getCapsules()
    capsuleDistances = [util.manhattanDistance(cp, successor.getPacmanPosition()) for cp in capsulePositions]
    capsuleDistances.append(2000)

    foodPositions = successor.getFood()
    foodDistances = [util.manhattanDistance(fp, successor.getPacmanPosition()) for fp in foodPositions]
    ...
    if len(foodDistances) != 0:
        features['closestFood'] = min(foodDistances)
        #features['farthestFood'] = max(foodDistances)

    features['numFood'] = successor.getNumFood()
    features['closestGhost'] = min(ghostDistances)
    #features['closesCapsule'] = min(capsuleDistances)
