    ...
    def observe(self, observation, gameState):
        ...
        if noisyDistance is None:
            ...
            for position in self.legalPositions:
                ...
                probOfNoisyGivenTrue = emissionModel[trueDistance]
                allPossible[position] = probOfNoisyGivenTrue * self.beliefs[position]
...
class ParticleFilter(InferenceModule):
    ...
    def initializeUniformly(self, gameState):
        ...
        numberOfLegalPosition = len(self.legalPositions)

        self.particles = []
        for i in range(0, self.numParticles):
            self.particles.append(self.legalPositions[i % numberOfLegalPosition])
    ...
    def observe(self, observation, gameState):
        ...
        if noisyDistance is None:
            self.particles = [self.getJailPosition() for _ in range(0, self.numParticles)]

        else:
            particleWeights = util.Counter()
            for particle in self.particles:
                trueDistance = util.manhattanDistance(pacmanPosition, particle)
                probabilityNoisyGivenTrue = emissionModel[trueDistance]
                particleWeights[particle] += probabilityNoisyGivenTrue
    ...
    def initializeParticles(self):
        ...
        for i in range(0, self.numParticles):
            ...
            for j in range(0, self.numGhosts):
                particle.append(self.legalPositions[(i*self.numGhosts + j) % numberOfLegalPosition])
    ...
    def observeState(self, gameState):
        ...
        for particle in self.particles:
            ...
            for ghostIndex in range(0, self.numGhosts):
                ...
                #    newParticle = self.getParticleWithGhostInJail(newParticle, ghostIndex)
                #else:
                trueDistance = util.manhattanDistance(pacmanPosition, newParticle[ghostIndex])
                weight *= emissionModels[ghostIndex][trueDistance]

            particleWeights[newParticle] += weight

        particleWeights.normalize()

        print particleWeights
        print emissionModels[0][util.manhattanDistance(pacmanPosition, (2,8))]
        print emissionModels[1][util.manhattanDistance(pacmanPosition, (5,6))]
